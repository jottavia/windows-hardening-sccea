...
Write-Host "==  MASTER WINDOWS HARDENING SCRIPT (v9.1) ==" -ForegroundColor Cyan
...
function Export-ComplianceVerification {
    ...
    $compliance = @{
        ...
        AccessControl = @{
            ...
            AccountLockoutPolicy = @{ LockoutThreshold = (Get-ItemProperty "HKLM:\SYSTEM\CurrentControlSet\Services\RemoteAccess\Policy" -Name "LockoutThreshold" -ErrorAction SilentlyContinue).LockoutThreshold }
            PasswordPolicy = @{ MinPasswordLength = (Get-ItemProperty "HKLM:\SYSTEM\CurrentControlSet\Services\RemoteAccess\Policy" -Name "MinPasswordLen" -ErrorAction SilentlyContinue).MinPasswordLen; MaxPasswordAge = (Get-ItemProperty "HKLM:\SYSTEM\CurrentControlSet\Services\RemoteAccess\Policy" -Name "MaxPasswordAge" -ErrorAction SilentlyContinue).MaxPasswordAge }
        }
        AuditAccountability = @{
            SecurityAuditing = @{ LogonEvents = (Get-WinEvent -FilterHashtable @{LogName='Security';ID=4624} -MaxEvents 1 -ErrorAction SilentlyContinue)-ne $null; LogoffEvents = (Get-WinEvent -FilterHashtable @{LogName='Security';ID=4634} -MaxEvents 1 -ErrorAction SilentlyContinue)-ne $null; AccountLockouts = (Get-WinEvent -FilterHashtable @{LogName='Security';ID=4740} -MaxEvents 1 -ErrorAction SilentlyContinue)-ne $null; PrivilegeUse = (Get-WinEvent -FilterHashtable @{LogName='Security';ID=4672} -MaxEvents 1 -ErrorAction SilentlyContinue)-ne $null }
            LogSettings = @{ SecurityLogSize = (Get-WinEvent -ListLog Security).MaximumSizeInBytes; SecurityLogRetention = (Get-WinEvent -ListLog Security).LogMode; SystemLogSize = (Get-WinEvent -ListLog System).MaximumSizeInBytes; ApplicationLogSize = (Get-WinEvent -ListLog Application).MaximumSizeInBytes }
            MonitoringTools = @{ WazuhAgent = (Get-Service -Name 'WazuhSvc' -ErrorAction SilentlyContinue)-ne $null; SysmonService = (Get-Service -Name 'Sysmon*' -ErrorAction SilentlyContinue)-ne $null }
        }
        ConfigurationManagement = @{
            WindowsUpdateConfig = @{ AutoUpdateEnabled = (Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update" -Name "AUOptions" -ErrorAction SilentlyContinue).AUOptions; LastUpdateCheck = (Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update\Results\Detect" -Name "LastSuccessTime" -ErrorAction SilentlyContinue).LastSuccessTime }
            ServicesConfig = @{ UnnecessaryServices = Get-Service | Where-Object {$_.StartType -eq 'Automatic' -and $_.Status -eq 'Stopped'} | measure-Object | select -ExpandProperty Count; RunningServices = (Get-Service | Where-Object Status -eq 'Running').Count }
            RegistryBaseline = @{ DefenderTamperProtection = Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows Defender\Features" -Name "TamperProtection" -ErrorAction SilentlyContinue; UACEnabled = (Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" -Name "EnableLUA" -ErrorAction SilentlyContinue).EnableLUA }
            ChangeTracking = @{ LastConfigChange = try{(Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update\Results\Install" -Name "LastSuccessTime" -ErrorAction Stop).LastSuccessTime}catch{"No update history found"}; SystemModificationDate = (Get-Item "C:\Windows\System32" -ErrorAction SilentlyContinue).LastWriteTime; RegistryLastModified = try{(Get-Item "HKLM:\SOFTWARE\Policies" -ErrorAction Stop).LastWriteTime.ToString('yyyy-MM-dd HH:mm:ss')}catch{"Registry key not accessible"}; RecentlyInstalledSoftware = Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*"|Where-Object {$_.InstallDate -gt (Get-Date).AddDays(-30).ToString("yyyyMMdd")}|select DisplayName,InstallDate,Publisher; SystemBootTime = (Get-CimInstance Win32_OperatingSystem).LastBootUpTime; ConfigurationBaseline = @{ LastHardeningRun=Get-Date -f 'o';ScriptVersion="v9.1-fixed";BaselineHash="Generated during hardening process"}}}
        SystemCommunications = @{
            Encryption = @{ BitLockerStatus = Get-BitLockerVolume | select MountPoint,VolumeStatus,ProtectionStatus; TLSSettings = @{ TLS12Enabled = (Get-ItemProperty "HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.2\Client" -Name "Enabled" -ErrorAction SilentlyContinue).Enabled; SSL3Disabled = (Get-ItemProperty "HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\SSL 3.0\Client" -Name "Enabled" -ErrorAction SilentlyContinue).Enabled -eq 0 }}
            ...
            RemoteAccess = @{ WinRMListeners = (Get-WSManInstance -ResourceURI winrm/config/listener -Enumerate -ErrorAction SilentlyContinue|measure-Object).Count; WinRMService = (Get-Service -Name 'WinRM' -ErrorAction SilentlyContinue).Status; WinRMStartupType = (Get-Service -Name 'WinRM' -ErrorAction SilentlyContinue).StartType; RDPEnabled = (Get-ItemProperty "HKLM:\System\CurrentControlSet\Control\Terminal Server" -Name "fDenyTSConnections" -ErrorAction SilentlyContinue).fDenyTSConnections; RDPService = (Get-Service -Name 'TermService' -ErrorAction SilentlyContinue).Status; RDPFirewallRule = (Get-NetFirewallRule -DisplayName "*Remote Desktop*"|Where-Object Enabled -eq $true|measure-Object).Count }}
        SystemIntegrity = @{
            MalwareProtection = @{ AntivirusEnabled = (Get-MpComputerStatus).AntivirusEnabled; RealTimeProtection = (Get-MpComputerStatus).RealTimeProtectionEnabled; DefinitionsUpToDate = (Get-MpComputerStatus).AntivirusSignatureAge -lt 7; QuarantineItems = (Get-MpThreatDetection|measure-Object).Count }
            ASRRules = @{ EnabledRules = (Get-MpPreference).AttackSurfaceReductionRules_Ids.Count; BlockMode = (Get-MpPreference).AttackSurfaceReductionRules_Actions -contains 1 }
            ApplicationControl = @{ SmartAppControl = Get-ItemProperty "HKLM:\SYSTEM\CurrentControlSet\Control\CI\Policy" -ErrorAction SilentlyContinue; WDACPolicy = Test-Path "$env:SystemRoot\System32\CodeIntegrity\SIPolicy.p7b" }}
        BackupVerification = @{
            URBackupClientStatus = (Get-Service -Name 'urbackupclientbackend' -ErrorAction SilentlyContinue).Status; URBackupClientStartType = (Get-Service -Name 'urbackupclientbackend' -ErrorAction SilentlyContinue).StartType; LastBackupDate = try{Get-ItemProperty "HKLM:\SOFTWARE\UrBackup\UrBackupClient" -Name "last_backup" -ErrorAction Stop}catch{"Registry key not found"}; BackupTestResults = Test-Path "C:\BackupRestoreTest.flag"; SystemRestorePoints = (Get-ComputerRestorePoint -ErrorAction SilentlyContinue|measure-Object).Count; URBackupProcessRunning = (Get-Process -Name 'urbackupclientbackend' -ErrorAction SilentlyContinue)-ne $null; BackupDiskSpace = try{Get-WmiObject -Class Win32_LogicalDisk|Where-Object DeviceID -eq "C:"|select @{Name="FreeSpaceGB";Expression={[math]::Round($_.FreeSpace/1GB,2)}}}catch{"Unable to retrieve disk space" }}
        ThirdPartyServices = @{
            InstalledThirdPartyServices = Get-Service | Where-Object {$_.ServiceName -notmatch '^(Microsoft|Windows|WinDefend|WSearch|Themes|BITS|EventLog|RpcSs|Dhcp|Dnscache|LanmanServer|LanmanWorkstation|Schedule|Spooler|AudioSrv|UxSms|ShellHWDetection|SamSs|PlugPlay|PolicyAgent|CryptSvc|TrustedInstaller|MpsSvc).*'} | select Name,Status,StartType,ServiceType
            WizerTrainingStatus = @{ Note="Manual verification required";CheckList="Training completion records in Security Binder";AnnualReview="DPA and SOC 2 compliance verification needed" }
            ExternalConnections = Get-NetTCPConnection|Where-Object {$_.RemoteAddress -notmatch '^(127\.0\.0\.1|::1|0\.0\.0\.0|169\.254\.)' -and $_.State -eq 'Established'}|select LocalAddress,LocalPort,RemoteAddress,RemotePort,OwningProcess
            NetworkShares = Get-SmbShare|Where-Object Name -ne "IPC$" | select Name,Path,ShareState,ShareType
...
function Test-BackupIntegrity {
    ...
    $backupTest = @{Timestamp=(Get-Date -f 'o');Computer=$env:COMPUTERNAME;URBackupClientRunning=(Get-Service -Name 'urbackupclientbackend' -ErrorAction SilentlyContinue).Status -eq 'Running';TestFileCreated=$false;TestFileHash=$null}
    try {
    ...
    $backupTestFile = Join-Path $script:logFolderPath "backup-integrity-test.json"
